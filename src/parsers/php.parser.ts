...
export class PHPParser extends AbstractParser {
  
  canParse(line: string): boolean {
    return ParserPatterns.commentDetection.php.test(line);
  }
  ...
  extractCommentBlock(lines: Array<string>, startLine: number): { content: string; endLine: number } | null {
    ...
    if(ParserPatterns.commentDetection.php.test(lines[startLine] || '')) {
      ...
      while(currentLine < lines.length) {
        ...
        if(ParserPatterns.commentBlockExtraction.php.end.test(line)) {
  ...
  parseCommentBlock(content: string, lineNumber: number, filePath: string): DocComment | null {
    ...
    for(const line of lines) {
      ...
      if(cleanLine.startsWith('@param')) {
        ...
        this.parsePHPDocReturn(cleanLine, docComment);
      } else if(cleanLine.startsWith('@throws')) {
        this.parsePHPDocThrows(cleanLine, docComment);
    ...
    try {
      for(let i = startLine; i < Math.min(startLine + 15, lines.length); i++) {
        ...
        if(functionMatch && functionMatch[1]) {
          ...
          if(!ParserPatterns.codeExtraction.php.keywords.includes(name)) {
            return name;
          }
        }
