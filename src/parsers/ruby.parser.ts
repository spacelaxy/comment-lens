...
export class RubyParser extends AbstractParser {
  ...
  extractCommentBlock(lines: Array<string>, startLine: number): { content: string; endLine: number } | null {
    ...
    if(ParserPatterns.commentDetection.ruby.test(lines[startLine] || '')) {
      ...
      while(currentLine < lines.length) {
        ...
        if(ParserPatterns.commentBlockExtraction.ruby.docComment.test(line || '')) {
  ...
  parseCommentBlock(content: string, lineNumber: number, filePath: string): DocComment | null {
    ...
    for(const line of lines) {
      ...
      if(cleanLine.startsWith('@param')) {
        this.parseRubyDocParam(cleanLine, docComment)
      } else if(cleanLine.startsWith('@return')) {
        this.parseRubyDocReturn(cleanLine, docComment)
      } else if(cleanLine.startsWith('@throws') || cleanLine.startsWith('@exception')) {
        this.parseRubyDocThrows(cleanLine, docComment)
  ...
  private parseRubyDocParam(paramText: string, docComment: DocComment): void {
  ...
  private parseRubyDocReturn(returnText: string, docComment: DocComment): void {
  ...
  private parseRubyDocThrows(throwsText: string, docComment: DocComment): void {
